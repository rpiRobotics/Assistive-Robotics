...
"""
Author: Burak Aksoy
Node: tf_mobile_base_imu_broadcaster
Description:
    Broadcasts to tf2 the trasformation between the IMU frame
    ...
    'oarbot_xxx_tf_mobile_base_imu.yaml'
    This yaml file also used to get the pre-calculated pose btw the mobile_base and the imu named
    'mobile_base2imu_pose'
    ...
    If is_fixed_z is true, the translational z value in the 'mobile_base2imu_pose' will be used.
...
class MobileBase2ImuTf():
    def __init__(self):
        rospy.init_node('tf_mobile_base_imu_broadcaster', anonymous=True)
        ...
        self.tf_imu_frame_id = rospy.get_param("~tf_imu_frame_id", "root_right_arm")
    
        self.mobile_base2imu_pose = rospy.get_param("~mobile_base2imu_pose") 
    ...
    def publish_tf_static(self):
        ...
        t.child_frame_id = self.tf_imu_frame_id

        t.transform.translation.x = self.mobile_base2imu_pose['position']['x']
        t.transform.translation.y = self.mobile_base2imu_pose['position']['y']
        t.transform.translation.z = self.mobile_base2imu_pose['position']['z'] + self.z_height
        ...
        t.transform.rotation.x = self.mobile_base2imu_pose['orientation']['x']
        t.transform.rotation.y = self.mobile_base2imu_pose['orientation']['y']
        t.transform.rotation.z = self.mobile_base2imu_pose['orientation']['z']
        t.transform.rotation.w = self.mobile_base2imu_pose['orientation']['w']

        self.tf_broadcaster_static.sendTransform(t)

if __name__ == '__main__':
    mobileBase2ImuTf = MobileBase2ImuTf()
    rospy.spin()