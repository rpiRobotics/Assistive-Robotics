...
class baseState(object):
    ...
    def control_timer(self, event):
        ...
        if self.base_vel is not None:
            ...
            dth = self.base_vel.angular*self.c_rate
            self.joint_state.position[self.joint_base_start] += dx
            self.joint_state.position[self.joint_base_start+1] += dy
            self.joint_state.position[self.joint_base_start+2] += dth
        if self.sup_vel is not None:
            dh = self.sup_vel*self.c_rate
            self.joint_state.position[self.joint_sup_start] += dh
    ...
    def ee_vel_cb(self,msg):
        ...
        if self.joint_state is None:
            return
        J = Oarbot.arm_jacobian(self.joint_state.position[self.joint_arm_start:self.joint_arm_start+6])
        nu = np.array([msg.angular.x,msg.angular.y,msg.angular.z,msg.linear.x,msg.linear.y,msg.linear.z])
        self.arm_joint_vel = np.linalg.pinv(J)*nu
    ...
    def set_pose(self,q):
        ...
        joint_array[self.joint_sup_start] = q[4]
        joint_array[self.joint_arm_start:self.joint_arm_start+6] = q[5:11]
        joint_array[self.joint_finger_start] = q[11]
        joint_array[self.joint_finger_start+2] = q[11]
        joint_array[self.joint_finger_start+4] = q[11]

        joint_msg.position = tuple(joint_array)
        joint_msg.stamp = rospy.Time.now()
